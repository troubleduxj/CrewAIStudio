...
class WorkflowBase(BaseModel):
    ...
    workflow_type: WorkflowType = Field(
        WorkflowType.SEQUENTIAL, description="工作流类型"
    )
    ...
    agents_config: List[Dict[str, Any]] = Field(
        default_factory=list, description="参与的Agent配置"
    )
    tasks_config: List[Dict[str, Any]] = Field(
        default_factory=list, description="任务配置"
    )

    # 执行配置
    max_execution_time: Optional[int] = Field(
        3600, ge=1, le=86400, description="最大执行时间（秒）"
    )
    ...
    schedule_config: Dict[str, Any] = Field(
        default_factory=dict, description="调度配置"
    )
    ...
    def validate_workflow_definition(cls, v):
        ...
        # 基本结构验证 - 只要求steps字段，connections字段可选
        if "steps" not in v:
            raise ValueError('Workflow definition must contain "steps" field')
...
class WorkflowUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=255, description="工作流名称"
    )
    description: Optional[str] = Field(None, description="工作流描述")
    ...
    workflow_definition: Optional[Dict[str, Any]] = Field(
        None, description="工作流定义JSON"
    )
    agents_config: Optional[List[Dict[str, Any]]] = Field(
        None, description="参与的Agent配置"
    )
    tasks_config: Optional[List[Dict[str, Any]]] = Field(None, description="任务配置")

    # 执行配置
    max_execution_time: Optional[int] = Field(
        None, ge=1, le=86400, description="最大执行时间（秒）"
    )
    ...
    trigger_conditions: Optional[List[Dict[str, Any]]] = Field(
...
class WorkflowResponse(WorkflowBase):
    ...
    execution_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="执行历史"
    )
    ...
    execution_result: Dict[str, Any] = Field(
        default_factory=dict, description="执行结果"
    )
